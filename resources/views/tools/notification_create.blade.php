...
@section('content')
    ...
    <div class="text-danger text-center"><b>{{$errors->error->first()}}</b></div>
        <form method="post" action="{{route('admin.tools.notification.store')}}">
            @csrf
            <div class="peers">
                <div class="col-12 col-sm-6">
                    <div class="form-group">
                        <label for="appid">AppId:</label>
                        <input class="form-control {{$errors->has('AppId')?'is-invalid':''}}" name="AppId" id="appid" value="{{old('AppId')}}" placeholder="app id" required/>
                        @if($errors->has('AppId'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="custom-select {{$errors->has('Type')?'is-invalid':''}}" name="Type" id="type" required>
                            <option value="0" {{old('Type')==0?'selected':''}}>Notification</option>
                            <option value="1" {{old('Type')==1?'selected':''}}>Update</option>
                            <option value="2" {{old('Type')==2?'selected':''}}>Promotion</option>
                            <option value="3" {{old('Type')==3?'selected':''}}>More App</option>
                        </select>
                        @if($errors->has('Type'))
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{$errors->has('StartDate')?'is-invalid':''}}" name="StartDate" id="start-date" value="{{old('StartDate')}}" placeholder="Start Date (Y-m-d)" required/>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{$errors->has('ExpiredDate')?'is-invalid':''}}" name="ExpiredDate" id="expired-date" value="{{old('ExpiredDate')}}" placeholder="Expired Date (Y-m-d)"/>
                ...
                <div class="col-12 col-sm-6">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{$errors->has('Notification.title')?'is-invalid':''}}" name="Notification[title]" id="notification-title" value="{{old('Notification')['title']??""}}" placeholder="Notification Title" required/>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{$errors->has('Notification.text')?'is-invalid':''}}" name="Notification[text]" id="notification-text" value="{{old('Notification')['text']??""}}" placeholder="Notification Text" required/>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" name="Notification[button][title]" id="notification-button-title" value="{{old('Notification')['button']['title']??""}}" placeholder="Notification Button Title"/>
                    </div>
                    <div class="form-group">
                        <label for="notification-button-link">Notification Button Link:</label>
                        <input class="form-control" name="Notification[button][link]" id="notification-button-link" value="{{old('Notification')['button']['link']??""}}" placeholder="Notification Button Link"/>
                    </div>
                </div>
            </div>
